...
FRAMING_STYLES = {
    "fun": {
        "name": "Fun & Engaging",
        "instruction": (
            "Reframe this prompt to be more engaging, playful, and fun - while keeping "
            "the core task exactly the same. Use collaborative framing "
            '("Let\'s figure out..."), add playful context, express curiosity or '
            "enthusiasm. Keep the same task, just make it more engaging."
        ),
    },
    "pirate": {
        "name": "Pirate Speak",
...
the same. The task shouldn't change, just the way it's asked.""",
    ...
    "expert": {
        "name": "Senior Expert",
        "instruction": (
            "Reframe this prompt as if a senior technical expert were asking a "
            "colleague. Use precise terminology, reference best practices, and frame "
            "it as a professional discussion. The core question stays the same but "
            "with expert-level framing."
        ),
    },
    "eli5": {
        "name": "Explain Like I'm 5",
        "instruction": (
            "Reframe this prompt as if a curious child were asking it. Use simple "
            'words, ask "why" and "how come", express wonder. The question should '
            "be simpler but asking for the same thing."
        ),
    },
    "formal": {
...
Reframed prompt:"""
    ...
    def get_response(self, prompt: str) -> str:
        ...
        system_prompt = (
            "You are a helpful assistant. Answer the user's question clearly " "and thoroughly."
        )
...
Please rate this response."""
    ...
    def run_trial(self, prompt: str, framing_key: str, num_judges: int = 3) -> Trial:
        ...
        self.framings.get(framing_key, {"name": framing_key})
    ...
    def run_experiment(
        ...
        if best.get("improvement_pct", 0) > 5:
            ...
            results.conclusion = (
                f"🏆 Best framing: '{results.best_framing}' " f"with {improvement:.1f}% improvement"
            )
    ...
    def generate_report(self, results: ExperimentResults) -> str:
        ...
        lines.append(
            "> Some framings (fun, expert, pirate, etc.) may produce better "
            "results than neutral prompts."
        )
        lines.append("")
        ...
        for i, (key, improvement) in enumerate(results.rankings):
            ...
            row = (
                f"| {emoji} {i+1} | {fr['framing_name']} | {improvement:+.1f}% "
                f"| {fr['framed_wins']} | {fr['raw_wins']} |"
            )
            lines.append(row)
...
Available built-in framings:
  ...
  skeptical, storyteller, confused
    ...
    if args.dry_run:
        ...
        transforms = args.trials
        responses = args.trials * 2
        judgments = args.trials * args.judges * 2
        print(
            f"  Per framing: {calls_per_framing} = {transforms} transforms + "
            f"{responses} responses + {judgments} judgments"
        )
        return
